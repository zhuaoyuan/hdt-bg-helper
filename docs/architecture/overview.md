...
**最后更新：** 2026-09-25（P0，初版，大部分组件尚未实现）
...
| 输入构造 InputBuilder | 由标准层重建 Bob's Buddy `Input`；逻辑对照 HDT 的 `BobsBuddyInvoker` / `BobsBuddyUtils` | P2 | ADR-0002（提议） |
...
外部 DLL（`HearthDb`、`BobsBuddy`、`Hearthstone Deck Tracker.exe` 等）从本机 HDT 安装目录引用，不提交到仓库（Q-005）。
