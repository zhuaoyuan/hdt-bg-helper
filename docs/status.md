...
- **Q-003 关闭 / ADR-0006 接受**（2026-09-25）：所有者确认本机个人非商业使用、调用公开 API、实时显示战力分位均为合理用途；仓库可公开但不得含 HDT / Bob's Buddy 二进制或反编译代码。据此接受 [ADR-0002](decisions/0002-bobsbuddy-as-primary-simulator.md)。
- **Q-001 / Q-005 调研**（2026-09-25）：Bob's Buddy 可在 HDT 外直接调用；.NET SDK 可编译插件。见 [`facts/bobsbuddy-public-api.md`](facts/bobsbuddy-public-api.md)、[`facts/licensing.md`](facts/licensing.md)。
- **本机环境与 ADR-0005**（2026-09-25）：以安装的修改版 HDT 1.58.1 为验证环境。
