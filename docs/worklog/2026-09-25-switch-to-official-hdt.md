...
## 追加：第一局验收（20260925_170955_cd944c）

- 所有者启用插件后打了 1 局单人酒馆战棋（17:09–17:25，9 个战斗回合后结束）。官方 HDT 1.58.3.8362 / BB 1.78.1 / 炉石 build 56608。
- HDT 日志：`[BgHelperDiag] loaded 0.1.0`，无 `probe unavailable`，无插件超时警告。HDT 自己每回合都打出了 Simulation Input/Output；回合 6 重跑了一次，插件的 `reRunCount=1` 对上了。
- `check_capture.py`：9/9 场战斗完整（开战快照、`hdt_bb`、Output、战后快照），0 错误，匿名化扫描通过。
- Q-002 首次实测成功：反射读到 `_input` 和 `Output`。9 个回合都有胜/平/负率，模拟次数均为 9996、`CompletedSimulations`。可用种族 5 个（机械 / 海盗 / 畸变 / 野兽 / 鱼人），无畸变。
- 性能（同一 UI 线程）：逐行回调中位 < 10 µs、p99 < 100 µs、最长 44 ms；实体快照最长 19 ms（第 9 回合 1125 个实体）；`hdt_bb` 序列化最长 24 ms。整局 65,110 行合计约 0.8 秒。
- 体积：`power.log.gz` 0.67 MB，`records.jsonl` 13.4 MB（其中实体快照约 10.1 MB，`hdt_bb` 约 3.3 MB）。实体数从开局 97 增到结束 1188。
- 原始行对照：HDT 的 Power 监视器只读 `PowerTaskList.DebugPrintPower` 等前缀（`LogWatcherManager.cs:43–46`），插件收到的就是其中转给 `OnPowerLogLine` 的那些。和当局 `Power_old.log` 比对：行数 65110 vs 65139（多出的 29 行在对局结束后）；对局里结构一致，差别只在 `Entity=` 里的玩家名（已被匿名化）。检查脚本已按这个过滤规则改过。
- 第一局验收通过，请所有者再打 3–5 局。字段级核对（Q-006 / Q-007 / 数据清单）等样本够了再做。
