...
- **状态：** approved（所有者 2026-09-25 对话批准，第 8 节三项均按建议确认）
...
第 7 节第 1–5 步已完成，代码在 `spikes/hdt-diag-logger/`，已部署到插件目录，还没有在 HDT 里实际运行。和方案的偏差：

- **不在插件里跟踪 BLOCK。** 原始行已经全部记下，BLOCK 上下文可以离线从 `power.log.gz` 重建。插件里只识别标签 `2022` / `3533` 的 `TAG_CHANGE` 和 `IsBattlegroundsCombatPhase` 的变化。
...
- **3 局验收（2026-09-26）通过**（27/27，0 错误，无超时）。`3533=0` 拍实体快照，`2022=0` 才有 BB Input（间隔 54–105 行）。新对局开头会倒出上一局 `_instances` 残留，分析须按行号过滤。结论见 `facts/diag-capture-measured.md`。
...
1. **用反射读 HDT 的内部字段**（`BobsBuddyInvoker._instances`、`_input` 等）：只读，不修改 HDT，也不反编译方法体。所有者确认这属于 ADR-0006 的合理用途，不另写 ADR。
2. **本地记录的匿名化方式**：按 3.5 节，所有者自己的 BattleTag 也一并替换。
3. **记录目录**：`%APPDATA%\HearthstoneDeckTracker\BgHelperDiag\`。aop
