...
| Q-010 | 本机安装的修改版 HDT（1.58.1）相对上游改了什么？是否改动了 `BobsBuddyInvoker`、日志格式或插件接口？ | 中 | 已知（2026-09-25，`facts/hdt-log-simulation-input.md`）：第三方"团子版"；日志只插入额外行，上游原有行格式不变；新增拔线、对战记录等功能，`BobsBuddyInvoker` 多了 `DoSimulation`、`RecordResult` 两个方法。**没发现改动模拟输入逻辑或插件接口的证据，但无法排除**：没有上游 1.58.1 二进制做方法体对比，也没有做成员级签名对比。现有日志都来自 1.57.12 及更早版本。剩余验证：成员级签名对比；拿到 1.58.1 的日志后复核 | ADR-0005、Q-002、Q-004、Q-012 | investigating |
| Q-011 | 离线批量模拟（P3）该用哪个版本的 `BobsBuddy.dll` 和 CardDefs？与采集时版本不一致时，结果会偏多少？ | 中 | 已知 1.78.8 删除了 1.76.0 中 Aberration 等下架卡牌的实现，HDT 运行时会下载最新 CardDefs（`facts/bobsbuddy-public-api.md`）。日志里没有记录 BB 版本，不能拿新版结果和日志里的旧 Output 直接比。验证方式：把同一批日志重建的场面分别在 1.76.0 和 1.78.8 下模拟，统计失败和偏差；确认采集时能否记录 BB 版本与 CardDefs 版本 | P2-T1、P3 | open |
| Q-012 | 继续用修改版 HDT 作为验证环境（ADR-0005）是否合适？ | 中 | 新发现的风险（2026-09-25）：修改版带拔线功能，上游注释写明暴雪要求 HDT 停止支持这类功能，存在账号风险；对战记录里有 372 场战斗因拔线没有实际结果，会污染战斗结果数据；模拟输入逻辑是否被改无法完全排除（Q-010）。需所有者决定：继续使用、改装官方版，或两者都装做对照。无论哪种，插件都不依赖修改版的任何行为 | ADR-0005、Q-010、P2-T5 | open |
